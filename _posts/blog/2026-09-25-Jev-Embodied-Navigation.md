...
Jev 本身还没有 VLN 论文，但近期三项工作从不同角度支持“**模型只做有界比较 / 验证，几何与执行留给机器人**”这一接口：
...
4. **在具身导航中，它的位置是战术 / 语义层。** 感知、几何、候选生成、安全与控制留在本地；System One 决策头比较已验证的候选、判断风险、决定是否升级。C²Nav、VerNav、O2C-Nav 从三个角度支持这一接口。
