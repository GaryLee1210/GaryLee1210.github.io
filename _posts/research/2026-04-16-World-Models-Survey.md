...
## 2.2 分类学：两大家族、四大范式与三个建模轴

"世界模型"一词目前被用来指代两类目标差异很大的系统。先把它们分开，后面的讨论才不会混淆。

### 两大家族：世界生成器 vs 面向决策的世界模型
...
### 家族 B 的四大应用范式（The Four Paradigms）
...
### 三个正交建模轴（Three-Axis Modeling Taxonomy）
...
## 2.3 系统定位：System 1 与 System 2 双系统协同
...
### 里程碑 2：Dreamer 系列与 RSSM（Hafner et al., arXiv 2023 / Nature 2025）——通用离散状态空间与 Minecraft 破局
...
### GENE-26.5（Genesis AI, 2026）：工业落地案例 {#paper-gene}
...
## 6.2 世界动作模型 (World Action Model, WAM)
...
### 图像/视频生成模型（Video Generation Foundations）
...
### 统一理解与生成模型（Unified Multimodal Foundations）
...
### 具身表示学习模型（Embodied Representation Foundations）
...
### 空间几何与 3D 基础模型（3D Spatial Foundations）
...
## 7.2 NVIDIA Cosmos：物理 AI 世界基础模型平台 {#cosmos}
...
## 7.3 Cosmos 3：全模态统一世界模型（2026） {#cosmos-3}
...
## 7.6 Video Generation Models in Robotics (2026) {#paper-videogen}
...
### 物理一致性与神经符号物理（Differentiable & Symbolic Physics）
...
### 时空 4D 感知与大型世界模型（Large World Models, LWM）
...
### 闭环安全性、不确定性量化与幻觉审计（Safety & Uncertainty）
...
### 长时程推演与常数内存架构（Long-Horizon Foresight）
...
### 失效感知动力学与真实世界泛化（Failure-Aware Dynamics & Sim2Real）
...
### 统一基础模型的全模态收敛（All-in-One Foundation Convergence）
